...
@override_settings(DEFAULT_FROM_EMAIL='noreply@example.com', EMAIL_SUPPRESS_RECIPIENTS=[])
class GiftVoucherConfirmationEmailTests(SimpleTestCase):
    ...
    @patch(
        'core.mail.superuser_notification_emails',
        return_value=['admin@example.com', 'buyer@example.com'],
    )
    @patch('payments.tasks.get_basket')
    def test_bccs_superusers_excluding_purchaser(self, get_basket, _superusers, email_message_cls):
        ...
        self.assertEqual(kwargs['bcc'], ['admin@example.com'])
        msg.send.assert_called_once_with(fail_silently=False)

    @patch('django.core.mail.EmailMessage')
    @patch('core.mail.superuser_notification_emails', return_value=[])
    @patch('payments.tasks.get_basket')
    def test_no_bcc_when_superuser_list_empty(self, get_basket, _superusers, email_message_cls):
        from payments.tasks import send_gift_voucher_confirmation_email
