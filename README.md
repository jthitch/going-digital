...
2. **Environment variables** (create `.env`; database is **MySQL/MariaDB** only):
   ...
   # Optional: Mailjet newsletter list sync (gd_customer.newsletter → contact list).
   # MAILJET_API_KEY=...
   # MAILJET_API_SECRET=...
   # MAILJET_API_BASE=https://api.mailjet.com/v3
   # MAILJET_NEWSLETTER_LIST_ID=12345678
   # MAILJET_WEBHOOK_TOKEN=optional-shared-secret
   # Then: python manage.py sync_mailjet_newsletter
   # Webhook: /newsletter/mailjet/webhook/?token=... (unsub → newsletter=0)
